...
def prepare_docs():
    ...
    token_vocab=[]
    sentences =[]
    for file in glob.glob(os.path.join("data", 'train.txt')):
...
def prepare_docs_df():
    ...
    doc_df = pd.read_csv('data/train.txt',header=None)
    doc_df.columns = ['text']
    for indx,item in enumerate(np.array(doc_df.text)[:40]):
        doc = nlp(item)
        token_vocab.append(lemmatize_doc(doc))
        sentences.append(sent.text for sent in doc.sents)
        for sent in doc.sents:
    ...
    print('info imported, sentences list created')
    print(time() - starttime)
...
corpus_tfidf,corpus,num_docs,dictionary,sentences = prepare_docs_df()
...
def save_df_as_json(num_docs,n,model_list,sentences,outdict):
    ...
    return df.to_json(orient='index'),df

jsonthing, dfthing = save_df_as_json(num_docs,7,[df_freq,df_tfidf],sentences,outdict=outdict)
